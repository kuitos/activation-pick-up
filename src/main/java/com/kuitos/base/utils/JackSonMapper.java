...
public class JackSonMapper {
    ...
    static {
         ...
         */
        classMapper.disable(DeserializationConfig.Feature.FAIL_ON_UNKNOWN_PROPERTIES);
        classMapper.configure(SerializationConfig.Feature.FAIL_ON_EMPTY_BEANS, false);
    }

    /** 从jsonNode对象中取值,如果未找到返回空字符串 */
    public static final String getValue(JsonNode node, String key){
        return node.get(key) != null ? node.get(key).getTextValue() : "";
    }

    /** 从jsonNode对象中查找对应的键值,如果未找到返回空字符串 */
    public static final String findValue(JsonNode node, String key){
        return node.findValue(key) != null ? node.findValue(key).getTextValue() : "";
    }
    ...
    public static final <T> T jsonNodeToClass(JsonNode jsonNode, Class<T> tClass) {
        ...
        try {
            req = tClass == String.class ? (T)jsonNode.toString() :
                    tClass == JsonNode.class ? (T)jsonNode : Map.class.isAssignableFrom(tClass) ? jsonMapper.readValue(jsonNode, tClass) : classMapper.readValue(jsonNode, tClass);
        } catch (Exception e) {
